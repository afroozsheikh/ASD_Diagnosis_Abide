...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Arguments for training GCN - kfold cross-validation"
    )
    parser.add_argument(
        "--features_path",
        type=str,
...
def main(args):
    ...
    kf = KFold(n_splits=args.k, shuffle=True, random_state=10)
    for i, (train_idx, val_idx) in enumerate(kf.split(data_list)):
        print(f"Fold {i}:")

        train_loader = DataLoader(
            [data_list[i] for i in train_idx], batch_size=args.batch_size, shuffle=True
        )
        val_loader = DataLoader(
            [data_list[i] for i in val_idx], batch_size=args.batch_size
        )
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            val_metrics = get_metrics(
                val_logits, val_y_true, args.last_activation, args
            )
        ...
        plot_loss(
            train_losses=train_losses, val_losses=val_losses, save_path=args.results
        )
        label_names = ["0", "1"]
