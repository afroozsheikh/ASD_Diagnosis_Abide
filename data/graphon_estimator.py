...
def align_graphs(
    graphs: List[np.ndarray], padding: bool = False
...
def estimate_target_distribution(
    probs: List[np.ndarray], dim_t: int = None
) -> np.ndarray:
    """
...
def estimate_graphon(
    graphs: List[np.ndarray], method, args
    ...
    graphon = cv2.resize(
        stepfunc, dsize=(args.r, args.r), interpolation=cv2.INTER_LINEAR
    )
    return stepfunc, graphon


def universal_svd(aligned_graphs: List[np.ndarray], threshold: float = 5) -> np.ndarray:
    """
...
def averaging_graphs(
    aligned_graphs: List[np.ndarray], trans: List[np.ndarray], ws: np.ndarray
) -> np.ndarray:
    """
    sum_k w_k * (Tk @ Gk @ Tk')
...
def node_cost_st(
    cost_s: np.ndarray, cost_t: np.ndarray, p_s: np.ndarray, p_t: np.ndarray
) -> np.ndarray:
    """
