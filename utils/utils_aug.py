...
def prepare_synthetic_dataset(dataset):
    ...
    for data in dataset:
        ...
        data.x = F.one_hot(degs.to(torch.int64), num_classes=max_degree + 1).to(
            torch.float
        )
        print(data.x.shape)
...
def align_graphs(
    graphs: List[np.ndarray], padding: bool = False, N: int = None
...
def estimate_graphon(dataset, graphon_estimator):
    ...
    for class_label in set(y_list):
        c_graph_list = [
            all_graphs_list[i] for i in range(len(y_list)) if y_list[i] == class_label
        ]

        aligned_adj_list, normalized_node_degrees, max_num, min_num = align_graphs(
            c_graph_list, padding=True, N=400
        )
...
def split_class_x_graphs(dataset):
    ...
    for class_label in set(y_list):
        ...
        c_node_x_list = [
            all_node_x_list[i] for i in range(len(y_list)) if y_list[i] == class_label
        ]
...
def universal_svd(
    aligned_graphs: List[np.ndarray], threshold: float = 0.14
) -> np.ndarray:
    """
...
def sorted_smooth(aligned_graphs: List[np.ndarray], h: int) -> np.ndarray:
    ...
    graphon = torch.nn.functional.conv2d(
        sum_graph, kernel, padding=0, stride=h, bias=None
    )
